...
`~/dev/repos/notes/WORKING_STATE.md` is the canonical agent working-state board for the human operator. Every agent run, regardless of which repo, branch, or worktree it is operating in, must keep it current so the operator has a single place to see all in-flight work. On start, read it; before finishing a turn that materially changes state (branch created, PR opened/updated, blocked, handed off, merged), update the relevant entry.

The board is a flat unordered list under a single `## Active` section — no tables, no instructions inline — so it stays human-readable as parallel work scales. Each entry is one top-level bullet keyed by the Jira ticket (raw link) with a shallow nested list of fields: Repo, Branch, Worktree (absolute path), PRs (raw Bitbucket links, comma-separated; omit field if none), Status (short phrase), Notes (path under `~/dev/repos/notes/` to the primary notes doc), Updated (`YYYY-MM-DD HH:MM`).

Do not remove an entry until the work is merged to the respective repo's main/master branch. A separate cleanup pass may sweep stale entries by checking each branch against its base; individual feature agents should not preemptively delete their own entries. The board is a status pointer only — decisions, analyses, design docs, and review feedback continue to live under `~/dev/repos/notes/core-notes/`, `~/dev/repos/notes/dss-notes/`, or dated scratch directories and are referenced from the entry's `Notes` field. The full schema and update rules live in `~/dev/repos/notes/AGENTS.md`. Updates to `WORKING_STATE.md` follow the notes repo's standard rule of always being committed (no pushing without explicit user request).
