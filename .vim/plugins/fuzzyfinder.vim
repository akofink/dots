...
"       nnoremap <silent> <C-]>      :FuzzyFinderTag!
...
"
"       nnoremap <C-p> :FuzzyFinderFile <C-r>=fnamemodify(getcwd(),
  ':p')<CR><CR>
  "
  "     Instead, if you want the directory of current buffer and not current
  "     directory:
  "
  "       nnoremap <C-p> :FuzzyFinderFile
  ...
  "       "~/project/**/src/*t*x*t*"
                             ...
                             'MruFile':{}, 'MruCmd':{}, 'FavFile':{}, 'Tag':{}, 'TaggedFile':{}}
                             ...
                             "   noremap  <silent> g]         :FuzzyFinderTag!
                             <C-r>=expand('<cword>')<CR><CR>
  ...
  " ChangeLog:
  "   2.13:
  "     - Fixed a bug that a directory disappeared when a file in that
  directroy
  ...
  "     - Fixed not to insert "zv" with "c/pattern<CR>" command in Normal
  mode.
  "     - Avoid the slow down problem caused by filereadable() check for the
  MRU
  ...
  "       history.
  "     - Fixed a bug that folds could not open with <CR> in a command-line
  when
  "       searching.
  ...
  "
  "   2.6.2:
  "     - Fixed not to miss changes in options when updates the MRU
  information.
  "
  "   2.6.1:
  ...
  "
  "   2.6:
  "     - Revived MRU-command mode. The problem with a command-line
  abbreviation
  "       was solved.
  ...
  "
  "   2.5:
  "     - Fixed the bug that a wrong initial text is entered after switching
  to a
  "       next mode.
  "     - Fixed the bug that it does not return to previous window after
  leaving
  "       Fuzzyfinder one.
  "
  "   2.4:
  "     - Fixed the bug that Fuzzyfinder fails to open a file caused by
  auto-cd
  "       plugin/script.
  "
  "   2.3:
  ...
  "     - Changed to inherit a typed text when switching a mode without
  leaving
  "       Insert mode.
  "     - Changed commands which launch explorers to be able to take a
  argument
  "       for initial text.
...
len(result)
  ...
  " 
  function!
s:FilterMatching(entries,
    key,
    pattern,
    index,
    limit)
return
s:FilterEx(a:entries,
    'v:val['''
    .
    a:key
    .
...
(len(s)
 ...
 -1)
  ...
  function!
s:ConvertWildcardToRegexp(expr)
  let
  re
  =
  escape(a:expr,
...
s:EscapeFilename(fn)
  ...
  escape(a:fn,
      ...
      "foo/.../bar/../../hoge"
      function!
      s:ExpandTailDotSequenceToParentDir(base)
      return
      substitute(a:base,
          '^\(.*[/\\]\)\?\zs\.\(\.\+\)\ze[^/\\]*$',
          \
      ...
      if
        a:evals_path_tail
          ...
          rate
          =
          s:EvaluateMatchingRate(s:SplitPath(matchstr(a:expr,
                  '^.*[^/\\]')).tail,
              \
                  ...
                  if
                    i_pattern
                      ...
                      * s:MATCHING_RATE_BASE
                      ...
                      function!
                      s:IsAvailableMode(mode)
                      return
                      exists('a:mode.mode_available')
                      &&
                      a:mode.mode_available
                      endfunction

                      function!
                      s:GetAvailableModes()
                      return
                      filter(values(g:FuzzyFinderMode),
                          's:IsAvailableMode(v:val)')
                      endfunction

                      function!
                      s:GetSortedAvailableModes()
                      let
                      modes
                      =
                      filter(items(g:FuzzyFinderMode),
                          's:IsAvailableMode(v:val[1])')
                      let
                      modes
                      =
                      map(modes,
...
wlidcards
return
split(join(map(s:CartesianProduct(lists),
        'expand(join(v:val,
            ""))'),
      "\n"),
    "\n")
...
len(a:excluded)
  ...
  function!
s:GetTagList(tagfile)
  return
  map(readfile(a:tagfile),
      'matchstr(v:val,
        ''^[^!\t][^\t]*'')')
      endfunction

      function!
      s:GetTaggedFileList(tagfile)
      execute
      'cd
      '
      .
      fnamemodify(a:tagfile,
        ':h')
      let
      result
      =
      map(readfile(a:tagfile),
        'fnamemodify(matchstr(v:val,
            ''^[^!\t][^\t]*\t\zs[^\t]\+''),
          '':p:~'')')
        cd
        -
...
range(min([len(a:i1.ranks),
  ...
  function!
s:GetCurrentTagFiles()
  return
  sort(filter(map(tagfiles(),
          'fnamemodify(v:val,
            '':p'')'),
          'filereadable(v:val)'))
      endfunction
  ...
  function!
    g:FuzzyFinderMode.Base.launch(initial_text,
        partial_matching,
        prev_bufnr,
        tag_files)
    "
    ...
    -1
    call
    s:InfoFileManager.load()
    if
      !s:IsAvailableMode(self)
        echo
        'This
        mode
        is
        ...
        call
        s:WindowManager.activate(self.make_complete_func('CompleteFunc'))
        call
        s:OptionManager.set('completeopt',
            'menuone')
        call
        s:OptionManager.set('ignorecase',
            self.ignore_case)
        ...
        for
          [lhs,
            ...
            \
              [
              self.key_ignore_case,
            self.to_str('on_switch_ignore_case()')
              ],
            \
              ]
              "
              ...
              function!
              g:FuzzyFinderMode.Base.on_cursor_moved_i()
              let
              ln
              =
              getline('.')
                ...
                !self.exists_prompt(ln)
                  ...
                  if
                    command
                      ...
                      call
                      setline('.',
                          self.restore_prompt(ln))
                      call
                      feedkeys(repeat("\<Right>",
                            len(getline('.'))
                            - len(ln)),
                          'n')
                      elseif
                        ...
                        the
                          ...
                          if
                            the
                              ...
                              function!
                              g:FuzzyFinderMode.Base.on_insert_leave()
                              let
                              text
                              =
                              getline('.')
                              ...
                              self.on_mode_leave()
                              call
                              self.empty_cache_if_existed(0)
                              call
                              s:OptionManager.restore_all()
                              call
                              s:WindowManager.deactivate()
                              ...
                              or
                                ...
                                fuzzyfinder.
                                if
                                  exists('s:reserved_switch_mode')
                                    let
                                    m
                                    =
                                    self.next_mode(s:reserved_switch_mode
                                        <
                                        0)
                                    call
                                    m.launch(self.remove_prompt(text),
                                        self.partial_matching,
                                        self.prev_bufnr,
                                        self.tag_files)
                                    unlet
                                    s:reserved_switch_mode
                                else
                                  if
                                    exists('s:reserved_command')
                                      call
                                      feedkeys(self.on_open(s:reserved_command[0],
                                            s:reserved_command[1]),
                                          'n')
                                      unlet
                                      s:reserved_command
                                      endif
                                      ...
                                      function!
                                      g:FuzzyFinderMode.Base.on_buf_enter()
                                      endfunction

                                      function!
                                      g:FuzzyFinderMode.Base.on_buf_write_post()
                                      endfunction

                                      function!
                                      g:FuzzyFinderMode.Base.on_command_pre(cmd)
                                      endfunction

                                      function!
                                      g:FuzzyFinderMode.Base.on_cr(index,
                                          check_dir)
                                      if
                                        pumvisible()
                                          call
                                          feedkeys(printf("\<C-y>\<C-r>=%s(%d,
                                          1)
                                                ?
                                                ''
                                                :
                                          ...
                                          a:index]
                                            ...
                                            if
                                              self.smart_bs
                                                ...
                                                function!
                                                g:FuzzyFinderMode.Base.on_mode_enter()
                                                endfunction

                                                function!
                                                g:FuzzyFinderMode.Base.on_mode_leave()
                                                endfunction

                                                function!
                                                g:FuzzyFinderMode.Base.on_open(expr,
                                                    mode)
                                                return
                                                [
                                                \
                                                ...
                                                \
                                                  ...
                                                  .
                                                  s:EscapeFilename(a:expr)
                                                  .
                                                  "\<CR>"
                                                  endfunction

                                                  function!
                                                  g:FuzzyFinderMode.Base.on_switch_mode(next_prev)
                                                  let
                                                  s:reserved_switch_mode
                                                  =
                                                  a:next_prev
                                                  ...
                                                  function!
                                                  g:FuzzyFinderMode.Base.on_switch_ignore_case()
                                                  let
                                                  &ignorecase
                                                  =
                                                  !&ignorecase
                                                  ...
                                                  list
                                                  function!
                                                  g:FuzzyFinderMode.Base.serialize_info()
                                                  let
                                                  header
                                                  =
                                                  self.to_key()
                                                  ...
                                                  list
                                                  function!
                                                  g:FuzzyFinderMode.Base.deserialize_info(lines)
                                                  let
                                                  header
                                                  =
                                                  self.to_key()
                                                  ...
                                                  map(filter(copy(a:lines),
                                                    ...
                                                    function!
                                                      g:FuzzyFinderMode.Base.complete(findstart,
                                                          base)
                                                      if
                                                        a:findstart
                                                          return
                                                          0
                                                          elseif
                                                          !self.exists_prompt(a:base)
                                                          ||
                                                          len(self.remove_prompt(a:base))
                                                          <
                                                          self.min_length
                                                          return
                                                          []
                                                          ...
                                                          for
                                                            expanded_base
                                                              in
                                                              s:ExpandAbbrevMap(self.remove_prompt(a:base),
                                                                  self.abbrev_map)
                                                              let
                                                              result
                                                              +=
                                                              self.on_complete(expanded_base)
                                                              endfor
                                                              call
                                                              sort(result,
                                                                  's:CompareRanks')
                                                              if
                                                                ...
                                                                call
                                                                  ...
                                                                  function!
                                                                  g:FuzzyFinderMode.Base.make_complete_func(name)
                                                                  execute
                                                                  printf("function!
                                                                      s:%s(findstart,
                                                                        base)\n"
  ...
  "
    ...
    {'base':'str',
      ...
      're':'\V\.\*str\.\*'}
  function!
    g:FuzzyFinderMode.Base.make_pattern(base)
    if
      self.partial_matching
        let
        wi
    ...
    etc.
      function!
      g:FuzzyFinderMode.Base.glob_ex(dir,
          file,
          excluded,
          index,
          matching_limit)
      ...
      if
        !exists('self.cache[key]')
          ...
          self.cache[key]
          =
          s:EnumExpandedDirsEntries(key,
              a:excluded)
          call
          s:ExtendIndexToEach(self.cache[key],
              1)
          endif
          echo
          'Filtering
          ...
          map(s:FilterEx(self.cache[key],
                ...
                a:matching_limit),
              return
              map(s:FilterMatching(self.cache[key],
                  'tail',
                  a:file,
                  a:index,
                  a:matching_limit),
    ...
    function!
    g:FuzzyFinderMode.Base.glob_dir_ex(dir,
        file,
        excluded,
        index,
        matching_limit)
    ...
    if
      !exists('self.cache[key]')
        ...
        self.cache[key]
        =
        filter(s:EnumExpandedDirsEntries(key,
              a:excluded),
            'len(v:val.suffix)')
        call
  ...
  call
    ...
    function!
    g:FuzzyFinderMode.Base.empty_cache_if_existed(force)
    if
      exists('self.cache')
        &&
        (a:force
         ||
         !exists('self.lasting_cache')
         ||
         !self.lasting_cache)
        unlet
        self.cache
        ...
        (type(self.cache)
         ...
         function!
         g:FuzzyFinderMode.Base.to_key()
         return
         filter(keys(g:FuzzyFinderMode),
             'g:FuzzyFinderMode[v:val]
             is
             self')[0]
         endfunction
         ...
         "
         returns
         'g:FuzzyFinderMode.{key}{.argument}'
         function!
         g:FuzzyFinderMode.Base.to_str(...)
         return
         'g:FuzzyFinderMode.'
         .
         self.to_key()
         ...
         function!
         g:FuzzyFinderMode.Base.extend_options()
         let
         n
         =
         filter(keys(g:FuzzyFinderMode),
         ...
         call
         extend(self,
             g:FuzzyFinderOptions[self.to_key()],
             'force')
         endfunction

         function!
         g:FuzzyFinderMode.Base.next_mode(rev)
         let
         modes
         =
         (a:rev
          ...
          s:GetSortedAvailableModes()
          :
          reverse(s:GetSortedAvailableModes()))
         let
         m_last
         =
         modes[-1]
           ...
           m
             ...
             if
               m
                 ...
                 function!
                 g:FuzzyFinderMode.Base.exists_prompt(in)
                 return
                 strlen(a:in)
                 >=
                 >strlen(self.prompt)
                 >&&
                 >a:in[:strlen(self.prompt)
                 >-1]
                 >==#
                 >self.prompt
                 endfunction
                 ...
                 function!
                 g:FuzzyFinderMode.Base.remove_prompt(in)
                 return
                 a:in[(self.exists_prompt(a:in)
                     ?
                     strlen(self.prompt)
                     :
                     0):]
                 ...
                 function!
                 g:FuzzyFinderMode.Base.restore_prompt(in)
                 let
                 i
                 =
                 0
                 while
                   i
                     ...
                     "-----------------------------------------------------------------------------
                     let
                     g:FuzzyFinderMode.Buffer
                     =
                     copy(g:FuzzyFinderMode.Base)

                     function!
                     g:FuzzyFinderMode.Buffer.on_complete(base)
                     let
                     patterns
                     =
                     self.make_pattern(a:base)
                     let
                     result
                     =
                     s:FilterMatching(self.cache,
                         'path',
                         patterns.re,
                         s:SuffixNumber(patterns.base),
                         0)
                     return
                     map(result,
                         's:FormatCompletionItem(v:val.path,
                           v:val.index,
                           v:val.path,
                           self.trim_length,
                           v:val.time,
                     ...
                     function!
                     g:FuzzyFinderMode.Buffer.on_open(expr,
                         mode)
                     "
                     attempts
                     to
                     ...
                     for
                       handling
                         ...
                         function!
                         g:FuzzyFinderMode.Buffer.on_mode_enter()
                         let
                         self.cache
                         =
                         map(filter(range(1,
                         ...
                         if
                           self.mru_order
                             call
                             s:ExtendIndexToEach(sort(self.cache,
                                   's:CompareTimeDescending'),
                                 1)
                             endif
                             endfunction

                             function!
                             g:FuzzyFinderMode.Buffer.on_buf_enter()
                             call
                             self.update_buf_times()
                             endfunction

                             function!
                             g:FuzzyFinderMode.Buffer.on_buf_write_post()
                             call
                             self.update_buf_times()
                             endfunction

                             function!
                             g:FuzzyFinderMode.Buffer.update_buf_times()
                             if
                               !exists('self.buf_times')
                                 let
                                 self.buf_times
                                 =
                                 {}
    ...
    let
    self.buf_times[bufnr('%')]
    =
    localtime()
    endfunction

    function!
    g:FuzzyFinderMode.Buffer.make_item(nr)
    return
    {
      \
        'index'
    ...
    "-----------------------------------------------------------------------------
    let
    g:FuzzyFinderMode.File
    =
    copy(g:FuzzyFinderMode.Base)

    function!
    g:FuzzyFinderMode.File.on_complete(base)
    let
    base
    =
    s:ExpandTailDotSequenceToParentDir(a:base)
    let
    patterns
    =
    map(s:SplitPath(base),
        'self.make_pattern(v:val)')
    let
    result
    =
    self.glob_ex(patterns.head.base,
        patterns.tail.re,
        self.excluded_path,
        s:SuffixNumber(patterns.tail.base),
        self.matching_limit)
    let
    result
    =
    ...
    if
      len(result)
        ...
        "-----------------------------------------------------------------------------
        let
        g:FuzzyFinderMode.Dir
        =
        copy(g:FuzzyFinderMode.Base)

        function!
        g:FuzzyFinderMode.Dir.on_complete(base)
        let
        base
        =
        s:ExpandTailDotSequenceToParentDir(a:base)
        ...
        result
        =
        self.glob_dir_ex(patterns.head.base,
            patterns.tail.re,
            self.excluded_path,
            s:SuffixNumber(patterns.tail.base),
            0)
        ...
        function!
        g:FuzzyFinderMode.Dir.on_open(expr,
            mode)
        return
        ':cd
        '
        ...
        \
          ...
          "-----------------------------------------------------------------------------
          let
          g:FuzzyFinderMode.MruFile
          =
          copy(g:FuzzyFinderMode.Base)

          function!
          g:FuzzyFinderMode.MruFile.on_complete(base)
          let
          patterns
          =
          self.make_pattern(a:base)
          ...
          function!
          g:FuzzyFinderMode.MruFile.on_mode_enter()
          let
          self.cache
          =
          copy(self.info)
  ...
  let
    self.cache
    =
    s:ExtendIndexToEach(self.cache,
        1)
    endfunction

    function!
    g:FuzzyFinderMode.MruFile.on_buf_enter()
    call
    self.update_info()
    endfunction

    function!
    g:FuzzyFinderMode.MruFile.on_buf_write_post()
    call
    self.update_info()
    endfunction

    function!
    g:FuzzyFinderMode.MruFile.update_info()
    "if
    !empty(&buftype)
    ||
    !filereadable(expand('%'))
    if
      !empty(&buftype)
        ...
        s:UpdateMruList(self.info,
            ...
            self.excluded_path)
        call
        s:InfoFileManager.save()
        endfunction

        "-----------------------------------------------------------------------------
        let
        g:FuzzyFinderMode.MruCmd
        =
        copy(g:FuzzyFinderMode.Base)

        function!
        g:FuzzyFinderMode.MruCmd.on_complete(base)
        let
        patterns
        =
        self.make_pattern(a:base)
        ...
        return
        map(result,
            's:FormatCompletionItem(v:val.command,
              v:val.index,
              v:val.command,
              self.trim_length,
              v:val.time,
        ...
        function!
        g:FuzzyFinderMode.MruCmd.on_open(expr,
            mode)
        redraw
        "
        use
        ...
        \
          ...
          function!
          g:FuzzyFinderMode.MruCmd.on_mode_enter()
          let
          self.cache
          =
          s:ExtendIndexToEach(map(copy(self.info),
                \
                '{
                "command"
                :
          ...
          function!
          g:FuzzyFinderMode.MruCmd.on_command_pre(cmd)
          call
          self.update_info(a:cmd)
          endfunction

          function!
          g:FuzzyFinderMode.MruCmd.update_info(cmd)
          call
          s:InfoFileManager.load()
          let
          self.info
          ...
          "-----------------------------------------------------------------------------
          let
          g:FuzzyFinderMode.FavFile
          =
          copy(g:FuzzyFinderMode.Base)

          function!
          g:FuzzyFinderMode.FavFile.on_complete(base)
          let
          patterns
          =
          self.make_pattern(a:base)
          ...
          function!
          g:FuzzyFinderMode.FavFile.on_mode_enter()
          let
          self.cache
          =
          copy(self.info)
  ...
  let
    ...
    function!
    g:FuzzyFinderMode.FavFile.add(in_file,
        adds)
    call
    s:InfoFileManager.load()
    ...
    "-----------------------------------------------------------------------------
    let
    g:FuzzyFinderMode.Tag
    =
    copy(g:FuzzyFinderMode.Base)

    function!
    g:FuzzyFinderMode.Tag.on_complete(base)
    let
    patterns
    =
    self.make_pattern(a:base)
    ...
    if
      len(result)
        ...
        function!
        g:FuzzyFinderMode.Tag.on_open(expr,
            mode)
        return
        [
        \
        ...
        \
          ...
          function!
          g:FuzzyFinderMode.Tag.find_tag(pattern,
              matching_limit)
          if
            !len(self.tag_files)
              return
                ...
                !exists('self.cache[key]')
                  ...
                  =
                  {
                    ...
                    \
                      'data'
                      :
                      s:Unique(s:Concat(map(copy(self.tag_files),
                              's:GetTagList(v:val)'))),
                    \
                  }
  endif
    ...
    list...'
    return
    s:FilterEx(self.cache[key].data,
        'v:val
        =~
        '
        .
    ...
    "-----------------------------------------------------------------------------
    let
    g:FuzzyFinderMode.TaggedFile
    =
    copy(g:FuzzyFinderMode.Base)

    function!
    g:FuzzyFinderMode.TaggedFile.on_complete(base)
    let
    patterns
    =
    self.make_pattern(a:base)
    ...
    result
    =
    self.find_tagged_file(patterns.re,
        self.matching_limit)
    if
      len(result)
        >=
        ...
        function!
        g:FuzzyFinderMode.TaggedFile.find_tagged_file(pattern,
            matching_limit)
        if
          !len(self.tag_files)
            return
    ...
    list...'
    return
    s:FilterEx(map(self.cache[key].data,
          'fnamemodify(v:val,
            '':.'')'),
          \
          'v:val
  ...
  function!
    s:OptionManager.set(name,
        value)
    call
    extend(self.originals,
        {
a:name
:
eval('&'
...
=
{
  ...
  function!
s:WindowManager.activate(complete_func)
  let
  self.prev_winnr
  =
winnr()
...
s:GetAvailableModes()
  ...
  lines
  =
readfile(expand(self.get_info_file()))
  catch
  /.*/ 
  return
  endtry
...
s:InfoFileManager.edit()
  ...
  <buffer>
  call
s:InfoFileManager.on_buf_write_cmd()
  augroup
  END
...
self.save()
  ...
  function!
s:InfoFileManager.get_info_version_line()
  return
  "VERSION\t206"
  endfunction

  function!
s:InfoFileManager.get_info_file()
  return
  g:FuzzyFinderOptions.Base.info_file
  endfunction

  function!
s:InfoFileManager.warn_old_info()
  echohl
  WarningMsg
  echo
  printf("==================================================\n"
                                                          ...
                                                          for
                                                            switching
                                                              ...
                                                              the
                                                                ...
                                                                aborted.
                                                                let
                                                                g:FuzzyFinderOptions.File.matching_limit
                                                                =
                                                                200
                                                                "-----------------------------------------------------------------------------
                                                                "
                                                                                                              ...
                                                                                                              switching
                                                                                                                ...
                                                                                                                if
                                                                                                                  zero
                                                                                                                    ...
                                                                                                                    set.
                                                                                                                    let
                                                                                                                    g:FuzzyFinderOptions.FavFile.mode_available
                                                                                                                    =
                                                                                                                    1
                                                                                                                    "
                                                                                                                    [Favorite-File
                                                                                                                    ...
                                                                                                                    string.
                                                                                                                    let
                                                                                                                    g:FuzzyFinderOptions.FavFile.prompt
                                                                                                                    =
                                                                                                                    '>FavFile>'
                                                                                                                    "
                                                                                                                    [Favorite-File
                                                                                                                    ...
                                                                                                                    for
                                                                                                                      a
                                                                                                                        ...
                                                                                                                        string.
                                                                                                                        let
                                                                                                                        g:FuzzyFinderOptions.FavFile.prompt_highlight
                                                                                                                        =
                                                                                                                        'Question'
                                                                                                                        "
                                                                                                                        [Favorite-File
                                                                                                                        ...
                                                                                                                        if
                                                                                                                          non-zero
                                                                                                                            ...
                                                                                                                            set.
                                                                                                                            let
                                                                                                                            g:FuzzyFinderOptions.FavFile.smart_bs
                                                                                                                            =
                                                                                                                            1
                                                                                                                            "
                                                                                                                            [Favorite-File
                                                                                                                            ...
                                                                                                                            for
                                                                                                                              switching
                                                                                                                                ...
                                                                                                                                mode.
                                                                                                                                let
                                                                                                                                g:FuzzyFinderOptions.FavFile.switch_order
                                                                                                                                =
                                                                                                                                60
                                                                                                                                "-----------------------------------------------------------------------------
                                                                                                                                "
                                                                                                                                            ...
                                                                                                                                            for
                                                                                                                                              switching
                                                                                                                                                ...
                                                                                                                                                the
                                                                                                                                                  ...
                                                                                                                                                  aborted.
                                                                                                                                                  let
                                                                                                                                                  g:FuzzyFinderOptions.Tag.matching_limit
                                                                                                                                                  =
                                                                                                                                                  200
                                                                                                                                                  "-----------------------------------------------------------------------------
                                                                                                                                                  "
                                                                                                                                                              ...
                                                                                                                                                              for
                                                                                                                                                                switching
                                                                                                                                                                  ...
                                                                                                                                                                  the
                                                                                                                                                                    ...
                                                                                                                                                                    aborted.
                                                                                                                                                                    let
                                                                                                                                                                    g:FuzzyFinderOptions.TaggedFile.matching_limit
                                                                                                                                                                    =
                                                                                                                                                                    200
                                                                                                                                                                    ...
                                                                                                                                                                    - {{{2
                                                                                                                                                                      call
                                                                                                                                                                        map(user_options,
                                                                                                                                                                            'extend(g:FuzzyFinderOptions[v:key],
                                                                                                                                                                              v:val,
                                                                                                                                                                              ''force'')')
                                                                                                                                                                            call
                                                                                                                                                                            map(copy(g:FuzzyFinderMode),
                                                                                                                                                                              'v:val.extend_options()')
                                                                                                                                                                            "
                                                                                                                                                                            }}}2
                                                                                                                                                                          ...
                                                                                                                                                                          which
                                                                                                                                                                            ...
                                                                                                                                                                            FuzzyFinderBuffer
                                                                                                                                                                            call
                                                                                                                                                                            g:FuzzyFinderMode.Buffer.launch
                                                                                                                                                                            (<q-args>,
                                                                                                                                                                             len(<q-bang>),
                                                                                                                                                                             bufnr('%'),
                                                                                                                                                                             s:GetCurrentTagFiles())
                                                                                                                                                                            ...
                                                                                                                                                                            FuzzyFinderMruFile
                                                                                                                                                                            call
                                                                                                                                                                            g:FuzzyFinderMode.MruFile.launch
                                                                                                                                                                            (<q-args>,
                                                                                                                                                                             len(<q-bang>),
                                                                                                                                                                             bufnr('%'),
                                                                                                                                                                             s:GetCurrentTagFiles())
                                                                                                                                                                            ...
                                                                                                                                                                            FuzzyFinderMruCmd
                                                                                                                                                                            call
                                                                                                                                                                            g:FuzzyFinderMode.MruCmd.launch
                                                                                                                                                                            (<q-args>,
                                                                                                                                                                             len(<q-bang>),
                                                                                                                                                                             bufnr('%'),
                                                                                                                                                                             s:GetCurrentTagFiles())
                                                                                                                                                                            ...
                                                                                                                                                                            FuzzyFinderFavFile
                                                                                                                                                                            call
                                                                                                                                                                            g:FuzzyFinderMode.FavFile.launch
                                                                                                                                                                            (<q-args>,
                                                                                                                                                                             len(<q-bang>),
                                                                                                                                                                             bufnr('%'),
                                                                                                                                                                             s:GetCurrentTagFiles())
                                                                                                                                                                            ...
                                                                                                                                                                            call
                                                                                                                                                                            g:FuzzyFinderMode.TaggedFile.launch(<q-args>,
                                                                                                                                                                                len(<q-bang>),
                                                                                                                                                                                bufnr('%'),
                                                                                                                                                                                s:GetCurrentTagFiles())
                                                                                                                                                                            command!
                                                                                                                                                                            ...
                                                                                                                                                                            -narg=?
                                                                                                                                                                            -complete=file
                                                                                                                                                                            FuzzyFinderAddFavFile
                                                                                                                                                                            call
                                                                                                                                                                            g:FuzzyFinderMode.FavFile.add(<q-args>,
                                                                                                                                                                                1)
                                                                                                                                                                            command!
                                                                                                                                                                            -bang
                                                                                                                                                                            -narg=0
