...
- Pi's agent-level retry defaults are managed in `templates/dot_pi/agent/settings.json`: retries are enabled with up to 8 attempts and a 2-second base delay, while provider-level retries remain disabled.
- Canonical Pi extensions live in the notes repo `agents/pi-extensions` directory and are symlinked into `~/.pi/agent/extensions`. The `disable-animations.ts` extension replaces the default animated working spinner with a static indicator.
